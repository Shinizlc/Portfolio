...
class ICO_BOT:
    ...
    ######my contract address
    # my_contract_address = '0x7977F3a21525F9C290Dc958a397ed23d1aEb5B3d'
    my_contract_address = '0x9C78Dc153d4f5c00946c82CE54db45A2662AB251'
...
##################################################
    ...
    def buy_token(self,token_in,token_out,amount_to_buy):
        ...
        if min_amount_out is not None and min_amount_out != 0:
            nonce = ICO_BOT.w3.eth.get_transaction_count(ICO_BOT.my_address_in_goeri)
            tx = ICO_BOT.my_contract.functions.buy(token_in,token_out,amount_to_buy,min_amount_out).build_transaction({'nonce':nonce,'gas':ICO_BOT.gas})
            signed_tx = ICO_BOT.w3.eth.account.signTransaction(tx, ICO_BOT.private_key)
            tx_hash = ICO_BOT.w3.eth.sendRawTransaction(signed_tx.rawTransaction)
            return ICO_BOT.w3.toHex(tx_hash)
        else:
    ...
    def sell_token(self,token_in,token_out,amount_to_buy):
        ...
        if min_amount_out is not None and min_amount_out != 0:
            nonce = ICO_BOT.w3.eth.get_transaction_count(ICO_BOT.my_address_in_goeri)
            tx = ICO_BOT.my_contract.functions.sell(token_in,token_out,amount_to_buy,min_amount_out).build_transaction({'nonce':nonce,'gas':ICO_BOT.gas})
            signed_tx = ICO_BOT.w3.eth.account.signTransaction(tx, ICO_BOT.private_key)
            tx_hash = ICO_BOT.w3.eth.sendRawTransaction(signed_tx.rawTransaction)
            return ICO_BOT.w3.toHex(tx_hash)
        else:
...
if __name__ == '__main__':
    ...
    # ico_bot1.sell_token('0xEf54F6d996A146A0dd0099228d5Ec7e24527e47a',ICO_BOT.weth_goerli,ico_bot1.get_token_balance('0xEf54F6d996A146A0dd0099228d5Ec7e24527e47a'))
