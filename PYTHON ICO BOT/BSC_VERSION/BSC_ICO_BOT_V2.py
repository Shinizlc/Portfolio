...
#######Adding funds to my contract ############################
    ...
    def is_owner_renounced(self, token):
        renounced_address = '0x0000000000000000000000000000000000000000'
        token_owner = self.my_contract.functions.renounced_owner(token).call({'from': self.my_address_in_bsc})
        if token_owner == renounced_address:
            return True
        else:
            return False
    @staticmethod
        ...
        while True:
            with requests.get(f'https://api.gopluslabs.io/api/v1/token_security/56?contract_addresses={token}') as link:
                if link.status_code == 200:
                    data = link.json()
                    if len(data['result'].keys())>0:
                        data = data['result'][token]
                        # pprint(data)
                        try:
                            if data['is_open_source'] and int(data['honeypot_with_same_creator']) == 0:
                                if any([int(data['anti_whale_modifiable']), int(data['can_take_back_ownership']), int(data['is_proxy']),int(data['is_honeypot']), int(data['is_mintable']), int(data['is_blacklisted']),int(data['is_anti_whale']), int(data['is_whitelisted']), int(data['trading_cooldown']),int(data['selfdestruct']), int(data['hidden_owner']), int(data['can_take_back_ownership'])]):
                                    return True
                                else:
                                    return False
                            else:
                            ...
                            logger.error(f'Security check returned not all values in the dictionary')
                            # continue
                            return True
                        ...
                        if tx_hash is not None:
                            ...
                            try:
                                ...
                                while True:
                                    ...
                                    if totalwork>=500 or price_diff<=-10 or price_diff>=70 or end_price <= ATH - ATH * Decimal(.2):
...
#     #it works
    ico_bot1 = ICO_BOT()
    # print(ICO_BOT.is_scam('0x6CDE79273FB7B4BB6c221F413E4565699b396748'))
    # print('0xdD65CB72F1E84B5fda9BF13753Ab7475F0848144'.lower())
    ico_bot1.main()
    # print(ico_bot1.is_owner_renounced('0x6886ed8Da98caFe10a5BFd3Dc23F8c207e7BDc22'))
    # print(ico_bot1.get_token_balance('0x4E4F479d99520EaFBd8F553675909E3C615A5178'))



    # ico_bot1.sell_token('0x72004294FCE4B9C0De7647D4Eb9fed245316fd0b','0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',ico_bot1.get_token_balance('0x72004294FCE4B9C0De7647D4Eb9fed245316fd0b'))
